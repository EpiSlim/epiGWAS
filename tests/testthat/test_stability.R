...
test_that("stabilityGLM shortens the path if the model saturates", {
    ...
    expect_type(stabilityGLM(X, Y, weights, family = "gaussian", lambda_min_ratio = 1e-05,
        short = TRUE), "double")
})
...
test_that("stabilityBIG correctly adapts to the length of the LASSO path",
    {
        n <- 200
        p <- 50
        X <- bigmemory::as.big.matrix(matrix(runif(n * p, min = 0, max = 1),
            nrow = n, ncol = p))
        Y <- rnorm(n)

        set.seed(468)
        aucs_short <- stabilityBIG(X, Y, family = "gaussian", ncores = 3,
            short = TRUE)
        set.seed(468)
        aucs_long <- stabilityBIG(X, Y, family = "gaussian", ncores = 3,
            short = FALSE)
...
test_that("stabilityBIG shortens the path if the model saturates", {
    ...
    expect_type(stabilityBIG(X, Y, family = "gaussian", ncores = 3, lambda_min_ratio = 1e-05,
        short = TRUE), "double")
})
