...
test_that("BOOST runs correctly in multithreaded and
  ...
  n_snps <- 200
  p_thresh <- 0.4
  A <- (runif(n_samples, min = 0, max = 1) < p_thresh) + (runif(n_samples, min = 0, max = 1) < p_thresh)
  X <- matrix((runif(n_samples * n_snps, min = 0, max = 1) < p_thresh) + (runif(n_samples * n_snps, min = 0, max = 1) < p_thresh), ncol = n_snps, nrow = n_samples)
...
test_that("BOOST computed scores are correct", {
  ...
  p_thresh <- 0.4
  A <- (runif(n_samples, min = 0, max = 1) < p_thresh) + (runif(n_samples, min = 0, max = 1) < p_thresh)
  X <- (runif(n_samples, min = 0, max = 1) < p_thresh) + (runif(n_samples, min = 0, max = 1) < p_thresh)
  Y <- (runif(n_samples, min = 0, max = 1) < 0.5)
