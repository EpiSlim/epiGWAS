...
test_that("sample_SNP has all of the required fields in the required length", {
  ...
  overlap_inter <- 0
  sampled <- sample_SNP(
    nX, nY, nZ12, clusters, MAF, thresh_MAF, window_size,
    overlap_marg, overlap_inter
  )

  expect_setequal(names(sampled), c(
    "target", "syner", "marginal",
    "inter1", "inter2"
  ))
...
test_that("gen_model is not affected by the type of the arguments mean and sd", {
  ...
  means <- list(rep(means[1], nX), rep(means[2], nX), rep(
    means[3],
    nY
  ), rep(means[4], nZ12))
  sds <- list(rep(sds[1], nX), rep(sds[2], nX), rep(sds[3], nY), rep(
    sds[4],
    nZ12
  ))
  set.seed(8487)
...
test_that("sim_phenotype is compatible with the whole pipeline", {
  ...
  model <- gen_model(nX, nY, nZ12, mean = rnorm(4), sd = runif(4,
    min = 0.5,
    max = 1.5
  ))
