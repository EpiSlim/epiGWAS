...
test_that("sample_SNP has all of the required fields in the required length", {
  ...
  overlap_marg <- 0
  overlap_inter <- 0
  sampled <- sample_SNP(nX, nY, nZ12, clusters, MAF, thresh_MAF, window_size, overlap_marg, overlap_inter)

  expect_setequal(names(sampled), c("target", "syner", "marginal", "inter1", "inter2"))
...
test_that("sim_phenotype is compatible with the whole pipeline", {
  ...
  n_samples <- 200
  p_thresh <- 0.4
  X <- matrix((runif(n_samples * length(clusters), min = 0, max = 1) < p_thresh) + (runif(n_samples * length(clusters), min = 0, max = 1) < p_thresh), ncol = length(clusters), nrow = n_samples)
  colnames(X) <- names(clusters)

  causal <- sample_SNP(nX, nY, nZ12, clusters, MAF, thresh_MAF, window_size, overlap_marg, overlap_inter)
  model <- gen_model(nX, nY, nZ12, mean = rnorm(4), sd = runif(4, min = 0.5, max = 1.5))
