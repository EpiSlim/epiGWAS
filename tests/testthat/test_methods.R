...
test_that("do.call is instructed for stabilityGLM", {
    ...
    expect_length(modified_outcome(A, X, Y, propensity, parallel = stability_mode,
        n_subsample = 20, n_lambda = 100), p)
    expect_length(shifted_outcome(A, X, Y, propensity, shift = 0.2, parallel = stability_mode,
        n_subsample = 20, n_lambda = 100), p)
    expect_length(normalized_outcome(A, X, Y, propensity, parallel = stability_mode,
        n_subsample = 20, n_lambda = 100), p)
    expect_length(robust_outcome(A, X, Y, propensity, parallel = stability_mode,
        n_subsample = 20, n_lambda = 100), p)
})
...
test_that("do.call is instructed for stabilityBIG", {
    ...
    expect_length(modified_outcome(A, X, Y, propensity, parallel = stability_mode,
        ncores = 3, n_subsample = 20, n_lambda = 100), p)
    expect_length(normalized_outcome(A, X, Y, propensity, parallel = stability_mode,
        ncores = 3, n_subsample = 20, n_lambda = 100), p)
    expect_length(shifted_outcome(A, X, Y, propensity, shift = 0.2, parallel = stability_mode,
        ncores = 3, n_subsample = 20, n_lambda = 100), p)
