...
## Case study
Now that the theoretical groundings of our methods are clear, we explain how to use them in practice for epistasis detection. For that purpose, we illustrate them on a synthetic dataset included with this package. Using the HAPGEN2 [@Su2011] software, we simulated $450$ SNPs on the $22^{nd}$ chromosome between the nucleotide positions $16061016$ [rs9617528] and $19976834$ [rs887201] in the GRCh37 coordinates.  The prior QC steps to control for rare variants ($\text{MAF} < 0.01$) and the Hardy–Weinberg equilibrium ($p < 10^{-6}$) have already been performed. The simulated genotypes are saved as an integer matrix. We also included their minor allele frequencies (MAFs).
...
To alleviate linkage disequilibrium around the target $A$ and avoid overfitting in the estimation of the propensity scores $\pi(A\lvert X)$, we remove a number of SNPs within a certain window of $A$. On each side of the target, the width of the window is three clusters. Compared to fixed-size windows, dynamic windows allow to better take into the genetic architecture. The clusters are the result of an unsupervised clustering procedure *e.g* hierarchical clustering:  
...
After the clustering, we can select the causal SNPs. Beside the target, the other causal variants are sampled outside of the LD window. In total, we sample $80$ SNPs that interact with the target, $20$ with marginal effects and $20$ SNP pairs with epistatic effects. Moreover, among the $80$ synergistic SNPs, $10$ also have separate marginal effects and another $5$ have additional epistatic effects. The `fast_HMM` function samples at most one causal SNP per cluster to avoid duplication of effects. Despite the high number of selected SNPs, the problem is still imbalanced with $80$ out of $414$ SNPs being causal.   
...
overlap_marg <- 10 # Number of SNPs interacting with the target in addition to their marginal effects
overlap_inter <- 5 # Number of SNPs interacting with the target in addition to their epistatic effects
```
...
### Epistasis detection
As the fastPHASE software cannot be included with this package, we directly provide the propensity scores vector. The results can be reproduced by running \code{fast_HMM} and the dimensionality of the latent space `n_state = 10` and the number of iterations for the EM algorithm `n_iter = 20`.
...
```{r}
stability_owl <- OWL(A, X, phenotype, propensity)
stability_modified <- modified_outcome(A, X, phenotype, propensity)
stability_shifted <- shifted_outcome(A, X, phenotype, propensity)
stability_normalized <- normalized_outcome(A, X, phenotype, propensity)
stability_robust <- robust_outcome(A, X, phenotype, propensity)
```

The last step is evaluate the model selection performance in terms of the areas under the receiver operating characteristic (ROC) and precision/recall curves (PR).
...
stability_scores <- join_scores(
  stability_owl, stability_modified, stability_shifted,
  stability_normalized, stability_robust
)
...
kable(aucs_table, caption = "Areas under the ROC and PR curves")  %>%
  ...
  row_spec(which(aucs_table$Method == "Robust modified outcome"), bold = T, color = "white", background = "#37648d")
```
